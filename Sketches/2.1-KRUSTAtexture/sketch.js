...
/*function exportHighResolutionPNG() {
  ...
  // Save as PNG
  save(myScaledCanvas, "KRUSTAlogo-Variation_" + num, '.png');
  num++;
...
function createControlPanel() {
  ...
  buttonCLEAR.position(width + 145, 60);
  buttonCLEAR.mousePressed(CLEAR);
  
  buttonPNG = createButton('Export PNG');
  buttonPNG.position(width + 140, 60);
  //buttonPNG.stroke(0);
  ...
  */
  ...
  /*let myTextArea = createElement('textarea','It is that special crispy sound');  
  myTextArea.attribute("rows","20");
  myTextArea.attribute("cols","32");
  myTextArea.position(width + 40, 160);
  myTextArea.size(270,100);*/

  createP('Select font').position(width + 40, 195).style('font-size','13px').style('font-family','sans-serif');
  selF = createSelect();
  selF.position(width + 40, 228);
  selF.option('Thin');
  selF.option('Regular');
  selF.option('Medium');
  selF.selected('Thin');
  ...
  createP('Select font size').position(width + 140, 195).style('font-size','13px').style('font-family','sans-serif');
  selFS = createSelect();
  selFS.position(width + 140, 228);
  selFS.option('Random');
  selFS.option('Small');
  selFS.option('Big');
  ...
  createP('Colour').position(width + 250, 195).style('font-size','13px').style('font-family','sans-serif');
  selC = createSelect();
  selC.position(width + 250, 228);
